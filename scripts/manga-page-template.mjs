export const siteUrl = "https://ichiwa.butchi.jp";
...
export function renderMangaPageHtml(indexHtml, manga, renderedPage) {
  const pageUrl = `${siteUrl}/manga/${encodeURIComponent(manga.slug)}`;
  ...
  const pageJsonLd = serializeJsonLd({
    ...
    author: { "@type": "Person", name: manga.author, url: authorUrl },
    sameAs: manga.xPostUrl,
    isPartOf: { "@type": "WebSite", name: "いちわ", url: `${siteUrl}/` },
  });
  ...
  const html = indexHtml
    ...
    .replace('<link rel="canonical" href="https://ichiwa.butchi.jp/" />', `<link rel="canonical" href="${pageUrl}" />`)
    ...
    .replace('<meta property="og:url" content="https://ichiwa.butchi.jp/" />', `<meta property="og:url" content="${pageUrl}" />`)
    ...
    .replace('<div id="root"></div>', `<div id="root">${renderedPage}</div>`);

  if (!html.includes(`canonical" href="${pageUrl}`) || !html.includes(`og:url" content="${pageUrl}`)) {
    throw new Error(`Could not update URL metadata for ${manga.slug}.`);
  }
